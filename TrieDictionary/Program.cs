...
Trie InitializeTrie(string[] words)
{
    ...
    // Iterate over each word in the provided array of words
    ...
    // Return the initialized Trie containing all the words
    return trie;
}
...
void PrintTrie(Trie trie)
{
    ...
    int rows = (wordCount + columnCount - 1) / columnCount; // Calculate the number of rows needed
    ...
    for (int i = 0; i < rows; i++)
    {
        for (int j = 0; j < columnCount; j++)
        {
            ...
            if (index < wordCount)
            {
                Console.Write($"{words[index],-15}"); // Adjust the width as needed
            }
        }
        Console.WriteLine();
    }
